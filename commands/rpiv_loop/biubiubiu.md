...
- **阶段 1**：读取需求摘要文件 `{brainstorm-summary-path}`，按照 RPIV create-prd 规范创建 PRD → `rpiv/requirements/prd-{feature-name}.md`。PRD 规范参考读取 `${CLAUDE_PLUGIN_ROOT}/commands/rpiv_loop/create-prd.md`
- **阶段 2**：等待 tech-research 任务完成（通过 TaskList 检查），然后按照 RPIV plan-feature 规范创建实施计划 → `rpiv/plans/plan-{feature-name}.md`。计划规范参考读取 `${CLAUDE_PLUGIN_ROOT}/commands/rpiv_loop/plan-feature.md`。代码库分析要做充分，计划要足够详细，让 Dev agent 无需额外调研就能实现
...
- 调研结果保存到 `rpiv/research-{feature-name}.md`
...
- **阶段 1**：分析需求，制定测试策略文档 `rpiv/validation/test-strategy-{feature-name}.md`
- **阶段 2**：基于 PRD 编写测试规格和验收标准（等 PRD 完成后开始）
- **阶段 3**：基于实施计划编写测试用例代码（与 Dev 并行）
- **阶段 4**：运行测试 + 代码审查。代码审查规范参考读取 `${CLAUDE_PLUGIN_ROOT}/commands/rpiv_loop/validation/code-review.md`。审查报告保存到 `rpiv/validation/code-review-{feature-name}.md`
...
- 按照计划中的逐步任务实现代码。执行规范参考读取 `${CLAUDE_PLUGIN_ROOT}/commands/rpiv_loop/execute.md`
