...
- **阶段 1**：读取需求摘要文件 `{brainstorm-summary-path}`，按照 RPIV create-prd 规范创建 PRD → `rpiv/requirements/prd-{feature-name}.md`。PRD 规范参考读取 `C:\Users\zqx\.claude\commands\rpiv_loop\create-prd.md`
...
- **阶段 4**：运行测试 + 代码审查。代码审查规范参考读取 `C:\Users\zqx\.claude\commands\rpiv_loop\validation\code-review.md`。审查报告保存到 `rpiv/validation/code-review-{feature-name}.md`
