...
核心不是"让 NBLM 做 PPT"，而是"架构师用 NBLM 作画笔"。
...
**架构师主导，NBLM 克制。** 架构师决定每一页的布局和每个元素的位置。NBLM 仅在现有图片无法表达意图时才启动——大多数页面保留官方原图或 Python 图表不动。一个 24 页 PPT 通常只有 3-5 页需要 NBLM 增强。

**先腾空间，再放图片。** 这是从实战中最痛的教训：绝不在已有内容上叠加图片。必须先用 Python 重构页面布局（缩小/移动/删除现有元素），为 NBLM 图片腾出明确的矩形区域，然后才插入图片。

**串行生产，时间换质量。** 每个 NBLM 任务一次只处理一张图。生成 3 个候选 → 架构师审查 → 择优 → 通过后才做下一张。慢，但每张图都经过审视。
...
2. 逐页判断图片策略：`[ORIGINAL]` / `[PYTHON]` / `[NBLM]`
...
同一张官方图出现在多个页面时，所有引用页均标记 `[ORIGINAL]`。不要为了"避免重复"而用 NBLM 替换——官方图的权威性比视觉多样性更重要。

3. 对每个 NBLM 任务，在 blueprint 中写明：
...
Part 1 越短越好。NBLM 对长描述的反应是添加更多细节，而非更精准地理解意图。如果你需要 5 句话才能描述清楚，说明这个概念对 NBLM 来说太复杂了——拆分成更小的图，或改用 Python 程序化绘制。
...
- **无标题**：Python 已经写了标题，NBLM 再加一个就重复了
- **无边框**：边框让图片看起来像独立贴纸而非页面的有机部分
- **无装饰文字**：NBLM 喜欢加段落解释，这些文字与 Python 文字风格不一致
- **限定颜色**：NBLM 不一定严格遵循 hex 值，但至少约束了色调方向
- **嵌入声明**：告诉 NBLM 这不是最终作品，而是更大页面中的一个元素
...
- NBLM 的图形生成对英文指令理解更精准，特别是否定指令（"NO title"）
- NBLM 会自动将图中标签翻译为中文，所以最终图片通常是中文的
...
| 情况 | 处理 |
|------|------|
| NBLM 生成超时 | `artifact poll {task_id}` 轮询，单次最长 10 分钟 |
